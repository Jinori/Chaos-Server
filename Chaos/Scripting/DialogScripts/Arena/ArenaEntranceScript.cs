    ...
    public override void OnNext(Aisling source, byte? optionIndex = null)
    {
        switch (Subject.Template.TemplateKey.ToLower())
        {
            case "rlyeh_pitfightconfirm":
            {
                ...
                var aislingChallenged = source.MapInstance
                                              .GetEntities<Aisling>()
                ...
                switch (optionIndex)
                {
                    case 1:
                    {
                        ...
                        aislingChallenged.Trackers.TimedEvents.AddEvent(
                            $"{source.Name.ToLower()}" + "denyPitFight",
            ...
            case "rlyeh_pitfightstart":
            {
                ...
                var aislingToChallenge = source.MapInstance
                                               .GetEntities<Aisling>()
                                               .FirstOrDefault(x => string.Equals(x.Name, name, StringComparison.CurrentCultureIgnoreCase));
                ...
                if (source.Trackers.TimedEvents.HasActiveEvent($"{aislingToChallenge.Name.ToLower()}_pitfight", out var pitfight))
                {
                    Subject.Reply(
                        source,
                        $"You can challenge {aislingToChallenge.Name} in {pitfight.Remaining.Humanize()}. You previously fought them in a pit fight.");
                ...
                if (source.Trackers.TimedEvents.HasActiveEvent($"{aislingToChallenge.Name.ToLower()}denyPitFight", out var @event))
                {
                    Subject.Reply(
                        source,
                        $"You can challenge {aislingToChallenge.Name} in {@event.Remaining.Humanize()}. They denied you previously.");
