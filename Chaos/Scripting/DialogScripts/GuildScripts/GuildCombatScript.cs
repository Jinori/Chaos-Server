...
public class GuildCombatScript : DialogScriptBase
{
    ...
    public GuildCombatScript(Dialog subject, IMonsterFactory monsterFactory) : base(subject)
    {
        ...
        SkillMappings = new Dictionary<string, Func<Aisling, List<Point>>>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "skillcombat_frontalcone_frontandcascaderange2", src => GetAoePoints(AoeShape.FrontalCone, src, 2, offsetX: 1) },
            { "skillcombat_frontalcone_frontandcascaderange3", src => GetAoePoints(AoeShape.FrontalCone, src, 3, offsetX: 1) },
            { "skillcombat_frontaldiamond_frontandcascaderange4", src => GetAoePoints(AoeShape.FrontalDiamond, src, 4, offsetX: 1) },
    ...
    private static List<Point> GetAoePoints(AoeShape shape, Aisling source, int range, int offsetX = 0, int offsetY = 0)
    {
        return shape.ResolvePoints(new AoeShapeOptions
        {
    ...
    private static List<Point> GetRandomPointsCentered(Point center, int width = 6, int height = 6, int count = 5)
    {
