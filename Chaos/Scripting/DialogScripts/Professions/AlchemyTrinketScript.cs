...
{
    /// <inheritdoc />
    public AlchemyTrinketScript(Dialog subject, IEffectFactory effectFactory, ISimpleCache simpleCache, IItemFactory itemFactory)
        : base(subject)
    {
    ...
    public override void OnNext(Aisling source, byte? optionIndex = null)
    {
        ...
        if (Subject.Template.TemplateKey.Equals("alchemytrinket_useonanother", StringComparison.CurrentCultureIgnoreCase))
        {
            ...
            if (aisling != null)
            {
                ...
                var newCraft = ItemFactory.Create(potion.ToString().ToLower());
                aisling.TryUseItem(newCraft);
                aisling.Animate(AlcUseAnimation);
                Subject.InjectTextParameters(potion.Humanize().Titleize());
        ...
        switch (Subject.Template.TemplateKey.ToLower())
        {
            ...
            case "alchemytrinket_userselectedapotion":
            {
                if (Subject.Context != null)
                {
                    ...
                    var cleanedContext = Subject.Context.ToString()?.Replace(" ", "");
            ...
            case "alchemytrinket_initial":
            {
                ...
                if (source.Trackers.TimedEvents.HasActiveEvent("AlchemyTrinketSetPotion", out var @event1) && source.Trackers.Enums.TryGetValue(out AlchemyTrinketSelection selection))
                {
            ...
            case "alchemytrinket_selectapotion":
            {
                ...
                if (source.Trackers.Flags.TryGetFlag(out AlchemyRecipes recipes))
                {
                    foreach ((var recipeKey, var recipeValue) in CraftingRequirements.AlchemyRequirements)
                    {
                        ...
                        if (recipes.HasFlag(recipeKey) && (playerRank == requiredRank) &&
                            (source.UserStatSheet.Level >= recipeValue.Level))
                        {
