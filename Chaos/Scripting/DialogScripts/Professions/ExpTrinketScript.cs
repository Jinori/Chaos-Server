...
public class ExpTrinketScript : DialogScriptBase
{
    ...
    public ExpTrinketScript(Dialog subject, ISimpleCache simpleCache)
        : base(subject)
    {
    ...
    private void CheckExpRates(Aisling source)
    {
        if (source.Trackers.Enums.TryGetValue(out ExpTimerStage stage) && (stage == ExpTimerStage.Tracking))
        {
            ...
            Subject.Reply(
                source,
                $"In the span of {duration.TotalMinutes:N0} minutes, you have harvested {expGained:N0} points of experience, weaving at a rate of about {expPerHour:N0} EXP per hour. May this knowledge fortify your path.");
        } else
    ...
    {
        if (!source.Legend.ContainsKey("jwlcrftng") && !source.IsGodModeEnabled())
        {
                ...
                foreach (var member in source.Group)
                    member.SendServerMessage(
                        ...
                        $"In the span of {duration.TotalMinutes:N0} minutes, you have harvested {expGained
                            ...
                            :N0} EXP per hour."
                        + Environment.NewLine
                        + "May this knowledge fortify your path.");
    ...
    private void StartExpTimer(Aisling source)
    {
        ...
        switch (stage)
        {
            ...
            case ExpTimerStage.Stopped:
                SetExpTimer(
                    source,
                    ExpTimerStage.Tracking,
                    source.UserStatSheet.TotalExp,
                    DateTime.UtcNow);
