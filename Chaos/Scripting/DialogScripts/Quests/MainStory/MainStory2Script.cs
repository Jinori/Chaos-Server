...
public class MainStory2Script(Dialog subject, IItemFactory itemFactory) : DialogScriptBase(subject)
{
        ...
        switch (Subject.Template.TemplateKey.ToLower())
        {
            case "mainstory_miraelis_initial":
            {
                ...
                if (source.Trackers.Enums.HasValue(MainStoryEnums.RetryServant))
                {
                    Subject.Reply(
                        source,
                        "Go back to Eingren Manor third floor with a group and take down the servant, figure out what he knows about where the Summoner went.");
                ...
                if (source.Trackers.Enums.HasValue(MainStoryEnums.CompletedPreMasterMainStory))
                {
                    Subject.Reply(
                        source,
                        "You've done very well this far Aisling. You've by far passed our expectations of you. Please come speak to me after you have mastered your class.");
                    ...
                    || source.Trackers.Enums.HasValue(MainStoryEnums.StartedSummonerFight))
                {
                    Subject.Reply(
                        source,
                        "It's great that you've found him again but this time we need to defeat him. Return to the Cthonic Remains and defeat the Summoner.");
                ...
                if (source.Trackers.Enums.HasValue(MainStoryEnums.CompletedTrials))
                {
                    if (source.UserStatSheet.Level < 97)
                    {
                        Subject.Reply(
                            source,
                            "We finished gathering the information and the task we must ask of you Aisling may be a bit too strong. Please return to us when you are stronger.");
                ...
                if (source.Trackers.Enums.HasValue(MainStoryEnums.SearchForSummoner))
                    Subject.Reply(
                        source,
                        "You have all the information we know now, the summoner is somewhere on the third floor of Eingren Manor. Please go search for him, make sure you bring a party, he is not going to be kind to visitors.");
