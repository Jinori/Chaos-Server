...
public class StartArenaRoomScript : DialogScriptBase
{
    private readonly ISimpleCache SimpleCache;

    /// <inheritdoc />
    public StartArenaRoomScript(Dialog subject, ISimpleCache simpleCache)
        : base(subject) =>
    ...
    public override void OnDisplaying(Aisling source)
    {
        ...
        if (allGroupMembersNearby)
        {
            ...
            foreach (var member in group)
            {
                var mapInstance = SimpleCache.Get<MapInstance>("undine_field_arena");
