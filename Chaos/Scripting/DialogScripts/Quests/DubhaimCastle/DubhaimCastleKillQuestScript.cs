...
public class DubhaimCastleKillQuestScript(Dialog subject, ILogger<TheSacrificeQuestScript> logger)
        ...
        switch (Subject.Template.TemplateKey.ToLower())
        {
            ...
            case "dubhaimcastlekillquest_initial":
            {
                ...
                if (hasStage && stage == DubhaimCastleKillQuestStage.DubhaimGhast)
                {
                    ...
                    {
                        Subject.Reply(source, "The ghast are formidable opponents sure, handle 10 of them.",
                            "hosk_initial");
                        return;
                    }
                ...
                {
                    if (!source.Trackers.Counters.TryGetValue("dubhaimgargoyle1counter", out var gargoyle1) ||
                        gargoyle1 < 10)
                    {
                        Subject.Reply(source,
                    ...
                    Subject.Reply(source,
                        $"That's better. The courtyard should be for all to enjoy.",
                        "hosk_initial");
                    source.TryGiveGamePoints(5);
                    ExperienceDistributionScript.GiveExp(source, tenPercent);
                    source.Trackers.TimedEvents.AddEvent("dubhaimcastlecd", TimeSpan.FromHours(22), true);
                ...
                {
                    if (!source.Trackers.Counters.TryGetValue("dubhaimgargoyle2counter", out var gargoyle2) ||
                        gargoyle2 < 10)
                    {
                        Subject.Reply(source,
                ...
                {
                    if (!source.Trackers.Counters.TryGetValue("dubhaimgargoyle3counter", out var gargoyle3) ||
                        gargoyle3 < 10)
                    {
                        Subject.Reply(source,
