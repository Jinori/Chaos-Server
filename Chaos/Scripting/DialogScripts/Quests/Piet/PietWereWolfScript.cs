...
public class PietWerewolfScript : DialogScriptBase
{
    ...
    public PietWerewolfScript(
        Dialog subject,
        ILogger<PietWerewolfScript> logger,
        ...
        switch (Subject.Template.TemplateKey.ToLower())
        {
            case "toby_initial":
            {
                if (source.Trackers.Enums.HasValue(WerewolfOfPiet.KilledandGotCursed))
                    Subject.Reply(
                        source,
                        "Thank you Aisling for knocking me out of Werewolf Form! I tried to cure or control it but I just can't. Appie says there is no cure! I'm sorry you're now effected with the curse, go speak to Appie, the Piet Magic Shop owner, he may be able to help you.");
            ...
            case "werewolf_initial":
            {
                if (source.UserStatSheet.Level < 71)
                    Subject.Reply(
                        source,
                        "Please don't mind that conversation, we will figure it out. You need to stay safe and not worry about our problems young one. Maybe when you're older, I'm sure that Werewolf will still be lurking in our town.");
            ...
            case "werewolf_return":
            {
                ...
                if (source.Trackers.Enums.HasValue(WerewolfOfPiet.SpawnedWerewolf2))
                {
                    Subject.Reply(
                        source,
                        "Looks like you took a beating, that werewolf is tough. You have to go back and get that flower.");
                ...
                if (source.Trackers.Enums.HasValue(WerewolfOfPiet.SpokeToWizard))
                {
                    Subject.Reply(
                        source,
                        "Why are you back here? Go search Shinewood Forest for the Werewolf's Woods and retrieve the flower we need for the cure.");
            ...
            case "werewolfwarp1":
            {
                ...
                if (source.Trackers.Enums.HasValue(WerewolfOfPiet.SpokeToWizard) && (source.Group != null))
                {
                    ...
                    if ((source.Group != null)
                        && source.Group.Any(
                            x => !x.Trackers.Enums.HasValue(WerewolfOfPiet.SpokeToWizard)
                                 && !x.Trackers.Enums.HasValue(WerewolfOfPiet.ReceivedCure)))
                        Subject.Reply(source, "Your group members must be on this part of the quest or finished the quest to help you.");
                }
            ...
            case "werewolfwarp2":
            {
                var groupmembers = source.MapInstance
                                         .GetEntities<Aisling>()
                                         .Where(
                                             x => (x.Group != null)
                                                  && x.WithinLevelRange(source)
                                                  && x.WithinRange(source)
                                                  && x.Group.Contains(source));
