...
public class PirateShipOverrunScript : DialogScriptBase
{
    ...
    /// <inheritdoc />
    public PirateShipOverrunScript(Dialog subject, ISimpleCache simpleCache, ILogger<PFQuestScript> logger)
        : base(subject)
    {
    ...
    public override void OnDisplaying(Aisling source)
    {
        switch (Subject.Template.TemplateKey.ToLower())
        {
            ...
            case "shipattack_initial":
            {
                ...
                if (source.Trackers.Enums.HasValue(PirateShip.CompletedShipAttack)
                    || source.Trackers.Flags.HasFlag(ShipAttackFlags.CompletedShipAttack))
                {
                ...
                if (source.Trackers.TimedEvents.HasActiveEvent("lynithpirateshipcd", out var cdtime))
                {
                    Subject.Reply(
                        source,
                        $"We just built our ship back up after the last attack. It'll be awhile before they return. (({cdtime.Remaining.Humanize()}))");
            ...
            case "shipattack_initial5":
            {
                ...
                if (group.All(member => member.Trackers.TimedEvents.HasActiveEvent("lynithpirateshipcd", out _)))
                {
                    ...
                    source.Client.SendServerMessage(
                        ...
                        "One of your members have attempted this quest too recently.");
                ...
                if (!group.All(member => member.UserStatSheet.Level > 70))
                {
                    ...
                    Subject.Reply(
                        source,
                        "I can't have you babysitting while you're defending my deck! Please make sure your group members are above level 71!");
