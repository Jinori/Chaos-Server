...
public class LearnSpellScript : DialogScriptBase
{
    ...
    private readonly Dictionary<string, List<string>> PureAbilities = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        {
            "darkstorm", new List<string>
            {
                "darkstorm"
            }
        },
        {
            "evasion", new List<string>
            {
                "evasion"
            }
        }
    ...
    private bool IsSpellLearnable(Spell spell, Aisling source)
    {
        ...
        if (SpellUpgradesByTemplateKey.Values.Any(
                upgradedspell => upgradedspell.Contains(spell.Template.TemplateKey, StringComparer.OrdinalIgnoreCase)))
        {
        ...
        if (SpellUpgradesByTemplateKey.TryGetValue(spell.Template.TemplateKey, out var upgrades))
            ...
            if (upgrades.Any(
                    upgrade
                        => source.SpellBook.Any(s => string.Equals(s.Template.TemplateKey, upgrade, StringComparison.OrdinalIgnoreCase))))
