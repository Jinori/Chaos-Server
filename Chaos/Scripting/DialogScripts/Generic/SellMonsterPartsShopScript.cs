...
public class SellMonsterPartsShopScript : DialogScriptBase
{
    ...
    public SellMonsterPartsShopScript(Dialog subject, IItemFactory itemFactory, ILogger<SellMonsterPartsShopScript> logger)
        : base(subject)
    {
    ...
    protected virtual void OnDisplayingAccepted(Aisling source)
    {
        ...
        if (total < amount)
        {
            Subject.Reply(source, $"You don't have enough {item.DisplayName}(s) to sell", "generic_sellmonsterpartsshop_initial");
        ...
        {
            Subject.Reply(source, "No appropriate Monster Extract found for this item.", "generic_sellmonsterpartsshop_initial");

            return;
        }

        // Handle the selling of the items and give the player the appropriate Monster Extracts
        var remainingExtracts = sellValue;
        var stackSize = 100; // Assuming 1000 is the maximum stack size for Monster Extracts

        while (remainingExtracts > 0)
        {
            var extractsToGive = Math.Min(remainingExtracts, stackSize);
        ...
        Subject.Reply(
            source,
            $"You sold {amount} {item.DisplayName}(s) for {sellValue} {extractTypeName}(s)",
