...
public class BuyMythicTokenShopScript : DialogScriptBase
{
    ...
    /// <inheritdoc />
    public BuyMythicTokenShopScript(
        Dialog subject,
    ...
    protected virtual void OnDisplayingAccepted(Aisling source)
    {
        ...
        switch (buyItemResult)
        {
            case ComplexActionHelper.BuyItemResult.Success:
                Logger.LogDebug(
                    "{@Player} bought {ItemCount} {@Item} from {@Merchant} for {TokenAmount} Mythic Tokens",
                    source,
                    amount,
                    item,
