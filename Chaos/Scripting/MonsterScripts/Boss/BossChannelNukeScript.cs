...
namespace Chaos.Scripting.MonsterScripts.Boss
{
    public sealed class BossChannelNukeScript : MonsterScriptBase
    {
        ...
        {
            Subject.MapInstance.ShowAnimation(new Animation { AnimationSpeed = 100, TargetAnimation = 840, TargetPoint = new Point(Subject.X, Subject.Y)});
            foreach (var point in SafePoints)
            {
                Subject.MapInstance.ShowAnimation(new Animation
                {
                    AnimationSpeed = 100,
                    TargetAnimation = 214,
                    TargetPoint = point,
        ...
        private void DamagePlayersNotOnSafePoints()
        {
            foreach (var player in Subject.MapInstance.GetEntities<Aisling>())
            {
                ...
                ApplyDamageScript.ApplyDamage(Subject, player, this, damage);
            }
        }
