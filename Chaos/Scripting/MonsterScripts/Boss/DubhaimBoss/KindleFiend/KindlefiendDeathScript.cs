...
public class KindlefiendDeathScript : MonsterScriptBase
{
    private readonly IItemFactory ItemFactory;
    protected IExperienceDistributionScript ExperienceDistributionScript { get; set; }

    /// <inheritdoc />
    public KindlefiendDeathScript(Monster subject, IItemFactory itemFactory)
        : base(subject)
    {
    ...
    public override void OnDeath()
    {
        ...
        if (rewardTargets is not null)
        {
            ...
            var bossReward = rewardTargets.Where(
                                              a => !a.Inventory.HasCountByTemplateKey("exptrinket", 1)
                                                   && !a.Bank.Contains("Sceallóg Taithí")
                                                   && a.Trackers.Enums.HasValue(ClassStatBracket.Grandmaster))
                                          .ToList();
            ...
            var expTrinket = ItemFactory.Create("exptrinket");
            rewardedAisling.GiveItemOrSendToBank(expTrinket);
            rewardedAisling.SendOrangeBarMessage("You received a unique trinket!");
        }
    }
}