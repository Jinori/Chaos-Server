...
public class SnagglesMonster(
    Monster subject,
    IMerchantFactory merchantFactory,
    IMonsterFactory monsterFactory,
    ISimpleCache simpleCache) : MonsterScriptBase(subject)
{
    ...
    private bool IsAdjacent(Point subjectPosition, Point aislingPosition)
        => ((Math.Abs(subjectPosition.X - aislingPosition.X) == 1) && (subjectPosition.Y == aislingPosition.Y))
    ...
    private bool IsAislingNearby()
    {
        ...
        return nearbyAislings.Any(aisling => IsAdjacent(new Point(Subject.X, Subject.Y), new Point(aisling.X, aisling.Y)));
    }

    private bool MovesTowardAisling(Point currentPosition, Point nextPosition, IEnumerable<Point> aislingPositions)
        => aislingPositions.Any(
            aislingPos => nextPosition.EuclideanDistanceFrom(aislingPos) < currentPosition.EuclideanDistanceFrom(aislingPos));
    ...
    public override void Update(TimeSpan delta)
    {
        ...
        var sweets = Subject.MapInstance
                            .GetEntities<GroundItem>()
                            .Where(x => x.Name is "Sweet Buns" or "Sugar")
                            .ToList();
        ...
        {
            var spawns = Subject.MapInstance
                                .GetEntities<Monster>()
                                .Where(
                                    x => (x.Template.TemplateKey == "snagglecottoncandy")
                                         && ((DateTime.UtcNow - x.Creation) >= TimeSpan.FromSeconds(15)))
                                .ToList();
        ...
        if (TagCount >= 3)
        {
            ...
            var spawns = Subject.MapInstance
                                .GetEntities<Monster>()
                                .Where(x => x.Template.TemplateKey == "snagglecottoncandy")
                                .ToList();
    ...
    public virtual void Wander()
    {
        ...
        var aislingPositions = nearbyAislings.Select(a => new Point(a.X, a.Y))
                                             .ToList();
        ...
        bool IsWithinSafeMapBounds(Point point)
            => (point.X >= (mapBounds.Left + 3))
               && (point.X <= (mapBounds.Right - 3))
               && (point.Y >= (mapBounds.Top + 3))
               && (point.Y <= (mapBounds.Bottom - 3));

        // Merge blocked points with Aisling positions
        var nearbyUnwalkablePoints = nearbyAislings.Concat(options.BlockedPoints)
                                                   .ToHashSet();
        ...
        var walkableDirections = possibleDirections.Where(
                                                       dir =>
                                                       {
        ...
        var safeDirections = walkableDirections.Where(
                                                   dir => !MovesTowardAisling(
                                                       new Point(Subject.X, Subject.Y),
                                                       Subject.DirectionalOffset(dir),
                                                       aislingPositions))
                                               .ToList();
