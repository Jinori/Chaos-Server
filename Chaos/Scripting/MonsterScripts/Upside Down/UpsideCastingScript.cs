...
public sealed class UpsideCastingScript : MonsterScriptBase
{
    ...
    private bool TryCastAoSpell(Spell spell, List<Creature> friendlyCreatures)
    {
        ...
        {
            var targetWithDebuff
                = friendlyCreatures.FirstOrDefault(creature => creature.Effects.Any(effect => effect.Name == debuffToRemove));
    ...
    public override void Update(TimeSpan delta)
    {
        ...
        var friendlyCreatures = Subject.MapInstance
                                       .GetEntitiesWithinRange<Creature>(Subject, 12)
                                       .Where(x => x.IsFriendlyTo(Subject) && !x.IsDead)
                                       .ToList();
