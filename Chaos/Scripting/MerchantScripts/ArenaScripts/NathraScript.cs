...
public sealed class NathraScript : MerchantScriptBase
{
    ...
    private void CheckAndUpdateCheckpoint()
    {
        ...
        if (CheckpointMap.TryGetValue(currentPoint, out var newState) && (NathraState != newState))
        {
            ...
            if (newState is CheckpointState.AtFirstPoint
                            or CheckpointState.AtSecondPoint
                            or CheckpointState.AtThirdPoint
                            or CheckpointState.AtFourthPoint)
    ...
    public override void Update(TimeSpan delta)
    {
        ...
        if (WalkTimer.IntervalElapsed)
        {
            ...
            var areDefendersPresent
                = playersInAoe.Any(x => x.Trackers.Enums.TryGetValue(out ArenaSide side) && (side == ArenaSide.Defender));

            var areOffensivePresent
                = playersInAoe.Any(x => x.Trackers.Enums.TryGetValue(out ArenaSide side) && (side == ArenaSide.Offensive));
            ...
            if (shouldMove
                && NathraState is CheckpointState.WalkingToFirstPoint
                                  or CheckpointState.WalkingToSecondPoint
                                  or CheckpointState.WalkingToThirdPoint
                                  or CheckpointState.WalkingToFourthPoint
                                  or CheckpointState.WalkingToGreenWinPoint)
