...
public class Summoner2MerchantScript : MerchantScriptBase
{
    ...
    /// <inheritdoc />
    public Summoner2MerchantScript(
        Merchant subject,
    ...
    private void HandleIdleState()
    {
        if (Subject.MapInstance
                   ...
                   .Any(
                       x => x.Trackers.Enums.HasValue(MainStoryEnums.SearchForSummoner2)
                            || x.Trackers.Enums.HasValue(MainStoryEnums.CompletedPreMasterMainStory)))
        {
            ...
            var players = Subject.MapInstance
                                 .GetEntities<Aisling>()
                                 .Where(x => x.Trackers.Enums.HasValue(MainStoryEnums.SearchForSummoner2) && !x.IsGodModeEnabled())
                                 .ToList();
    ...
    private void HandleSummonerConversation(TimeSpan delta)
    {
        ...
        } else if (!HasSummonedTauren && DialogueTimer.IntervalElapsed)
        {
            ...
            var players = Subject.MapInstance
                                 .GetEntities<Aisling>()
                                 .Where(x => x.Trackers.Enums.HasValue(MainStoryEnums.FoundSummoner2))
                                 .ToList();
        ...
        } else if (!HasCreantsEscaped && DialogueTimer.IntervalElapsed)
        {
            if (!Subject.MapInstance
                        ...
                        .Any(x => (x.Name != "Tauren") && (x.Name != "Phoenix") && (x.Name != "Medusa") && (x.Name != "Shamensyth")))
                return;

            foreach (var player in Subject.MapInstance
                                          .GetEntities<Aisling>()
                                          .Where(x => !x.IsGodModeEnabled() && x.Trackers.Enums.HasValue(MainStoryEnums.SpawnedCreants)))
            {
        ...
        } else if (!HasSaidDialog4 && DialogueTimer.IntervalElapsed)
        {
            var aislings = Subject.MapInstance
                                  .GetEntities<Aisling>()
                                  .Where(
                                      x => x.Trackers.Enums.HasValue(MainStoryEnums.StartedSummonerFight)
                                           || x.Trackers.Enums.HasValue(MainStoryEnums.CompletedPreMasterMainStory))
                                  .ToList();
