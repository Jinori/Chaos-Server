    ...
    : MerchantScriptBase(subject)
{
    ...
    private bool GameShouldComplete()
        => AnnouncedStart.HasValue
           && (DateTime.UtcNow.Subtract(AnnouncedStart.Value)
                       .TotalMinutes
               >= 1)
           && AnnouncedOneMinuteTimer;

    private void GatherAislingsAtCompletion()
        => AislingsAtCompletion = Subject.MapInstance
                                         .GetEntities<Aisling>()
                                         .Where(x => x.BetGoldOnTwentyOne)
                                         .ToList();

    private void HandleFinalCountdown()
    {
        if (AnnouncedStart.HasValue
            && (DateTime.UtcNow.Subtract(AnnouncedStart.Value)
                        .TotalSeconds
                >= 50)
            && !CountdownStarted)
        {
            ...
            if (DateTime.UtcNow.Subtract(LastCountdownTime)
                        .TotalSeconds
                >= 1)
            {
