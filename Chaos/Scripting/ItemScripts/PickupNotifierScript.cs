...
namespace Chaos.Scripting.ItemScripts
{
    public class PickupNotifierScript : ItemScriptBase
    {
        ...
        {
            var message = originalCount.Equals(1) ? $"You picked up {Subject.DisplayName}." : $"You picked up {originalCount.ToWords()} of {Subject.DisplayName}.";
        ...
        private void NotifyGroupMembers(Aisling aisling, int originalCount, List<Aisling> nearbyPlayers)
        {
            if (aisling.Group is { Count: > 1 })
            {
                ...
                foreach (var member in groupMembersInRange)
                {
                    ...
                    {
                        member.SendOrangeBarMessage(originalCount > 1 ? $"{aisling.Name} has picked up {originalCount.ToWords()} {Subject.DisplayName}." : $"{aisling.Name} has picked up {Subject.DisplayName}.");   
                    }
                }
            }
        }
