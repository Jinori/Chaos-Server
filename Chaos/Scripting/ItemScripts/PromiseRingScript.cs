...
{
    public class PromiseRingScript(Item subject, IDialogFactory dialogFactory, IMerchantFactory merchantFactory) 
        : ItemScriptBase(subject)
    {
        ...
        private static Aisling? FindTargetPlayer(Aisling source) =>
            source.MapInstance.GetEntitiesWithinRange<Aisling>(source, 1)
        ...
        private static bool AreFacingEachOther(Aisling source, Aisling target)
        {
            ...
            return source.Direction == targetDirectionToSource && 
                   target.Direction == sourceDirectionToTarget;
        }
