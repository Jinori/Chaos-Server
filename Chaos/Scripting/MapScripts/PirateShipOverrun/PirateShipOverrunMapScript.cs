...
public class PirateShipOverrunMapScript : MapScriptBase
{
    ...
    {
        IRectangle shipRectangle1 = new Rectangle(
            5,
            8,
            5,
            8);

        IRectangle shipRectangle2 = new Rectangle(
            10,
            9,
            7,
            9);

        IRectangle shipRectangle3 = new Rectangle(
            17,
            11,
            15,
            7);

        var monstersOnShip = Subject.GetEntities<Monster>()
                                    .Where(x => shipRectangle1.Contains(x) && shipRectangle2.Contains(x) && shipRectangle3.Contains(x))
                                    .ToList();
    ...
    private ScriptState GetNextStateAfterDelayedStart()
    {
        foreach (var aisling in Subject.GetEntities<Aisling>())
        {
            ...
            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave2))
                return ScriptState.SpawningThirdWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave3))
                return ScriptState.SpawningFourthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave4))
                return ScriptState.SpawningFifthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave5))
                return ScriptState.SpawningSixthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave6))
                return ScriptState.SpawningSeventhWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave7))
                return ScriptState.SpawningEighthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave8))
                return ScriptState.SpawningNinthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave9))
                return ScriptState.SpawningTenthWave;

            if (aisling.Trackers.Enums.HasValue(PirateShip.FinishedWave10))
    ...
    {
        IRectangle exclusionRectangle = new Rectangle(
            4,
            6,
            29,
            13);
        ...
        for (var i = 0; i < count; i++)
            if (Subject.Template.Bounds.TryGetRandomPoint(
                    pt => !Subject.IsWall(pt) && !IsPointInRectangle(pt, exclusionRectangle),
                    out var point))
            {
                ...
                if (excessVitality > 0)
                {
                    ...
                    attrib.Add(adjust);
                    attrib.SkillDamagePct += (int)(excessVitality * 0.01); // 1% of excess vitality added to Skill Damage

                    attrib.SpellDamagePct += (int)(excessVitality * 0.01); // 1% of excess vitality added to Spell Damage
                }
