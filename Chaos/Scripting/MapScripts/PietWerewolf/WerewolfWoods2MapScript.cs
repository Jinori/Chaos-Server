...
public class WerewolfWoods2MapScript : MapScriptBase
{
    ...
    public WerewolfWoods2MapScript(
        MapInstance subject,
    ...
    private void HandleScriptState2()
    {
        switch (State)
        {
            case ScriptState2.Dormant:
                if (Subject.GetEntities<Aisling>()
                           .Any(
                               a => a.Trackers.Enums.HasValue(WerewolfOfPiet.SpokeToWizard)
                                    || a.Trackers.Enums.HasValue(WerewolfOfPiet.RetryWerewolf)))
    ...
    private void SpawnMonsters(string monsterType)
    {
        ...
        var masterwerewolf = Subject.GetEntities<Merchant>()
                                    .Where(x => x.Name == "Master Werewolf")
                                    .ToList();
