...
public class PietWerewolfMapScript : MapScriptBase
{
    ...
    public PietWerewolfMapScript(MapInstance subject,
    ...
    public override void Update(TimeSpan delta)
    {
        ...
        {
            var werewolf = Subject.GetEntities<Merchant>().Any(x => x.Template.TemplateKey == "werewolfToby");

            if (werewolf)
                return;
            
            if (!Subject.GetEntities<Aisling>().Any(x => x.Trackers.Enums.HasValue(WerewolfOfPiet.StartedQuest)))
                return;
