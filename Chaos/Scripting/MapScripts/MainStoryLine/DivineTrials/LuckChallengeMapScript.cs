...
public class LuckChallengeMapScript : MapScriptBase
{
    ...
    private ScriptState GetNextStateAfterDelayedStart()
    {
        foreach (var aisling in Subject.GetEntities<Aisling>())
        {
            ...
            if (aisling.Trackers.Enums.HasValue(LuckTrial.SucceededFirst))
                return ScriptState.Spawning2;

            if (aisling.Trackers.Enums.HasValue(LuckTrial.SucceededSecond))
                return ScriptState.Spawning3;

            if (aisling.Trackers.Enums.HasValue(LuckTrial.SucceededThird))
    ...
    private void SpawnMerchants()
    {
        ...
        var totalMerchants = DoorsuccessCount + DoorfailCount;
        var maxIndex = Math.Min(totalMerchants, SpawnPoints.Count);

        // Spawn door success merchants
        for (var i = 0; (i < maxIndex) && (i < DoorsuccessCount); i++)
        {
        ...
        for (var i = DoorsuccessCount; (i < maxIndex) && (i < totalMerchants); i++)
        {
            var point = SpawnPoints[i];
            ...
            switch (State)
            {
                case ScriptState.Dormant:
                    if (Subject.GetEntities<Aisling>()
                               .Any(a => a.Trackers.Enums.HasValue(MainStoryEnums.StartedSecondTrial)))
                ...
                case ScriptState.Spawned1:
                {
                    ...
                    if (!Subject.GetEntities<Merchant>()
                                .Any(x => x.Template.TemplateKey != "trialofluckchest"))
                    {
                        CurrentRound++;
                        State = ScriptState.DelayedStart;
                    }
