...
public class SacrificeChallengeMapScript(MapInstance subject, ISimpleCache simpleCache, IMonsterFactory monsterFactory)
    : MapScriptBase(subject)
{
    ...
    private bool CheckAllMonstersCleared(SacrificeTrial trial)
    {
        if (!Subject.GetEntities<Monster>()
                    .Any(m => !m.Script.Is<PetScript>() && !m.Script.Is<SacrificeZoe>()))
        {
            foreach (var aisling in Subject.GetEntities<Aisling>())
    ...
    private ScriptState2 GetNextStateAfterDelayedStart()
    {
        foreach (var aisling in Subject.GetEntities<Aisling>())
        {
            ...
            if (aisling.Trackers.Enums.HasValue(SacrificeTrial.FinishedSecond))
                return ScriptState2.SpawningThirdWave;

            if (aisling.Trackers.Enums.HasValue(SacrificeTrial.FinishedThird))
                return ScriptState2.SpawningFourthWave;

            if (aisling.Trackers.Enums.HasValue(SacrificeTrial.FinishedFourth))
    ...
    private void HandleScriptState()
    {
        switch (State)
        {
            case ScriptState2.Dormant:
                if (Subject.GetEntities<Aisling>()
                           .Any(a => a.Trackers.Enums.HasValue(SacrificeTrial.StartedTrial)))
