...
public sealed class HuntingGroundsDetermineWinnerScript : MapScriptBase
{
    ...
    private void AssignRandomTargets()
    {
        ...
        {
            var possibleTargets = playerList.Where(x => !x.Equals(player) && !AssignedTargets.ContainsValue(x))
                                            .ToList();
    ...
    {
        var possibleTargets = ActivePlayers.Where(x => !x.Equals(player) && !AssignedTargets.ContainsValue(x))
                                           .ToList();
    ...
    {
        var respawnedPlayers = DeathList.Where(player => player.IsAlive)
                                        .ToList();
    ...
    {
        var deadPlayers = Subject.GetEntities<Aisling>()
                                 .Where(x => !x.IsAlive)
                                 .ToList();
        ...
        foreach (var deadPlayer in deadPlayers)
        {
            ...
            if (deadPlayer.Trackers.LastDamagedBy is Aisling killer)
            {
                ...
                foreach (var player in AssignedTargets.Where(kvp => kvp.Value.Equals(deadPlayer))
                                                      .ToList())
