...
public class CasinoMonsterRaceLaneWaterScript : ReactorTileScriptBase
{
    ...
    /// <inheritdoc />
    public CasinoMonsterRaceLaneWaterScript(ReactorTile subject, ILogger<CasinoMonsterRaceLaneWaterScript> logger)
        : base(subject)
    {
        Logger = logger;
    ...
    public override void OnWalkedOn(Creature source)
    {
        ...
        foreach (var aisling in AislingsAtCompletion)
            if (aisling.MonsterRacingLane is "Water")
        ...
        switch (AislingsThatWon.Count)
        {
            case 1:
            {
                ...
                foreach (var aisling in AislingsAtCompletion)
                    aisling.SendActiveMessage($"{winner.Name} wins on lane Water!");
            ...
            {
                foreach (var aisling in AislingsAtCompletion)
                    aisling.SendActiveMessage("Lane Water won! Nobody picked Lane Water!");
