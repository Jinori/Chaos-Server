...
public class CasinoMonsterRaceLaneThreeScript : ReactorTileScriptBase
{
    ...
    /// <inheritdoc />
    public CasinoMonsterRaceLaneThreeScript(ReactorTile subject)
        : base(subject) { }
    ...
    public override void OnWalkedOn(Creature source)
    {
        ...
        if (Subject.DistanceFrom(source) <= 0)
        {
            ...
            foreach (var aisling in AislingsAtCompletion)
                if (aisling.MonsterRacingLane is 3)
            ...
            if (AislingsThatWon.Count == 1)
            {
                ...
                foreach (var aisling in AislingsAtCompletion)
                    aisling.SendActiveMessage($"{winner.Name} wins on lane three!");
