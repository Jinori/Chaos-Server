...
public class CrHorrorEntranceScript : ReactorTileScriptBase
{
    ...
    public override void OnWalkedOn(Creature source)
    {
        ...
        var allMembersHaveQuestFlag = group.All(member =>
            member.Trackers.Flags.HasFlag(QuestFlag1.CrHorror) && member.WithinLevelRange(source));
        ...
        else
        {
            // Send a message to the Aisling
            aisling.Client.SendServerMessage(ServerMessageType.OrangeBar1,
                "Make sure everyone is within level range and has quest.");
            // Warp the source back
            var point = source.DirectionalOffset(source.Direction.Reverse());
            source.WarpTo(point);
        }
