...
public class QueenOctopusRoomScript : ReactorTileScriptBase
{
    ...
    public override void OnWalkedOn(Creature source)
    {
        ...
        var allMembersHaveQuestEnum = aisling.Group.All(
            member =>
            {
                if ((member.Trackers.Enums.TryGetValue(out QueenOctopusQuest stage) && (stage == QueenOctopusQuest.QueenSpawned))
                    || (stage == QueenOctopusQuest.Complete))
                    return true;

                if ((stage != QueenOctopusQuest.SpokeToMaria)
                    || (!member.Inventory.Contains("Red Pearl") && !member.Inventory.Contains("Coral Pendant")))
                {
